...
log_dir=/workspace/data/A10_compare
mkdir -p ${log_dir}
...
model_name_arr=("opt-66b" "opt-66b" "opt-66b" "opt-66b" "opt-66b" "opt-66b")
input_len_arr=(512 512 512 512 512 512)
out_len_arr=(8 8 8 8 8 8)
bs_arr=(60 60 60 60 72 72)
num_bs_arr=(4 4 8 8 4 4)
percent_arr=("0 100 0 100 0 100" "0 50 0 0 0 100" "0 100 0 100 0 100" "0 50 0 0 0 100" "0 100 0 100 0 100" "0 50 0 0 0 100")
...
for(( i=0;i<${#model_name_arr[@]};i++)) do
   ...
   args_str="--model ${model_path} ${off_dir_args} --path _DUMMY_  --percent ${percent} --gpu-batch-size ${bs} --num-gpu-batches ${num_bs}  --prompt-len ${input_len} --gen-len ${out_len}  --compress-weight --compress-cache";
