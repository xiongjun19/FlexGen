...
def prof_collect(inputs, model, gen_len):
    ...
    for i in range(1):
        ...
        with torch.no_grad():
            output_ids = model.generate(input_ids=inputs, **generate_kwargs)
    p_stop()
...
def run_generation(model_name, batch_size, prompt_len, gen_len, cut_gen_len,
                   ...
                   output_file, pkl_file, no_log, verbose, cpu_log_path):
    # Load tokenizer
    ...
    prof_collect.set_path(cpu_log_path)
    prof_collect(input_ids, model, execute_gen_len)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cpu_log_path", type=str, default="tmp_cpu")
    ...
    run_generation(args.model, args.batch_size, args.prompt_len, args.gen_len,
                   ...
                   args.no_log, args.verbose, args.cpu_log_path)
