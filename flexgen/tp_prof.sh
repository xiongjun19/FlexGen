...
for(( i=0;i<${#tp_arr[@]};i++)) do
   ...
   CUDA_VISIBLE_DEVICE=4,6 nsys profile  -c cudaProfilerApi -f true --stats true  -o ${gpu_log} python  tp_flex_opt_prof_new.py --model facebook/opt-30b  --percent 0 100 100 0 100 0 --gpu-batch-size 1 --num-gpu-batches 1 --prompt-len 128 --gen-len 8 --tp_num $tp --path _DUMMY_;
   echo "finished one"
done
