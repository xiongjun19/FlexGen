...
# Print the nodeid
echo "CXL-OFFLOAD Using CPU NODE ID: $nodeid"
    ...
    case "$1" in
        --cxl-offload)
            if [ $MEM_SET -eq 0 ]; then
                ...
                numactl --membind=$MEM_SET --cpunodebind=${nodeid} $PYTHON flex_opt.py --model $MODEL --offload-dir /workspace/data/flex_offload_dir --path _DUMMY_ --percent 0 100 0 100 0 100 --gpu-batch-size ${batch_size} --num-gpu-batches 4 --prompt-len 512 --gen-len 8 --compress-weight --compress-cache --log-file ${log_file}
                echo "stop" > message.txt
        ...
        --all-offload)
            ...
            if [ $MEM_SET -eq 0 ]; then
                ...
                numactl --interleave=0,1,2 $PYTHON flex_opt.py --model $MODEL --offload-dir tmp/data/flex_offload_dir --path _DUMMY_ --percent 0 100 0 100 0 100 --gpu-batch-size ${batch_size} --num-gpu-batches 4 --prompt-len 512 --gen-len 8 --compress-weight --compress-cache --log-file ${log_file}
                echo "stop" > message.txt
            fi
            shift
            ;;
        ...
        --disk-offload)
            ...
            if [ $MEM_SET -eq 0 ]; then
                ...
                cgexec -g memory:${CGROUP_NAME} $PYTHON flex_opt.py --model $MODEL --offload-dir /tmp/data/flex_offload_dir --path _DUMMY_ --percent 0 0 0 0 0 100 --gpu-batch-size ${batch_size} --num-gpu-batches 4 --prompt-len 512 --gen-len 8 --compress-weight --compress-cache --log-file ${log_file}
                echo "stop" > message.txt
            fi
            shift
            ;;
